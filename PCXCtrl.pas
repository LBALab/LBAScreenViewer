...
type
  ...
  protected
    	...
    	BitmapInfo: PBitmapInfo); virtual; abstract;
  ...
  protected
		...
		procedure MakeBMIColors(Stream: TStream;
    	BitmapInfo: PBitmapInfo); override;
    function GetPaletteSize: integer; override;
 	public
		function DecodeStream(AStream: TStream;
			PCXHeader: TPCXHeader; AImage: TMemoryStream): integer; override;
  end;
...
begin
  ...
  begin
    ...
    if (Bpp = 8) and (NPlanes = 1) then FColorModel := Color256
    	else if (Bpp = 1) and (NPlanes = 4) then FColorModel := Color16
      	else Exit;
    ImageHeight := longint(Ymx) - longint(Ymn) + 1;
    ImageWidth := longint(Xmx) - longint(Xmn) + 1;
  end;
  Image := TMemoryStream.Create;
  if FColorModel = Color16 then Decoder := TDecoder16.Create
  	else Decoder := TDecoder256.Create;
  try
...
begin
  ...
  if Image = nil then inherited LoadFromStream(Stream)
  	else inherited LoadFromStream(Image);
end;
...
begin
  ...
  if BufferIndex = 0 then
    if Count > ReadCount then Stream.ReadBuffer(Buffer, ReadCount)
    	else Stream.ReadBuffer(Buffer, Count);
...
{ TDecoder256 }
function TDecoder256.DecodeStream(AStream: TStream;
	PCXHeader: TPCXHeader; AImage: TMemoryStream): integer;
var
	Bmi: PBitMapInfo;
  Bmf: TBitmapFileHeader;
	Image: TMemoryStream;
...
begin
  ...
  AStream.Seek(128, 0);
    ...
    try
    	Image.Position := longint(ImageOffset) +
      	(longint(LineCount) * longint(ImageBytesPerLine));
...
{R-}
procedure TDecoder16.MakeBMIColors(
	Stream: TStream; BitmapInfo: PBitmapInfo);
var
	b: byte;
  i: integer;
  Header: TPCXHeader;
...
function TDecoder16.DecodeStream(AStream: TStream;
	PCXHeader: TPCXHeader; AImage: TMemoryStream): integer;
var
	Bmi: PBitMapInfo;
  Bmf: TBitmapFileHeader;
